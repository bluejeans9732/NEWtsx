...
export default async function Edit(props: { params: { id: string } }) {
  ...
  return (
    <div className='flex items-center justify-center h-screen bg-gradient-to-br from-gray-50 to-gray-100'>
        <div className='w-1/3 h-[60%] p-4 rounded-xl  shadow-xl bg-white'>
            <h4 className='mb-3 text-center text-2xl text-cyan-900 font-bold'>글 수정</h4>
            <form action="/api/post/edit" method="POST">
                <div className='flex mt-10 mb-10'>   
                    <div className='mr-2 text-cyan-900 font-bold p-2'>제목 : </div> 
                    <input name="title" className='ml-4 flex border-2 p-2 ' defaultValue={post?.title}></input>
                </div>
                <div  className='flex mt-10 mb-10'>
                    <div className='mr-2 text-cyan-900 font-bold p-2'>내용 : </div> 
                    <input name="content" className='mb-10 ml-4 flex p-2 border-2' defaultValue={post?.content}></input>
                </div>
